...
class Configs(BaseConfigs):
    ...
    def get_w(self, batch_size: int):
        ...
        if torch.rand(()).item() < self.style_mixing_prob:
            ...
            w2 = w2[None, :, :].expand(
                self.n_gen_blocks - cross_over_point, -1, -1)
    ...
    def step(self, idx: int):
        ...
        if (idx + 1) % self.log_generated_interval == 0:
            tracker.add('generated', torch.cat(
                [generated_images[:6], real_images[:3]], dim=0))
